...
import edu.brown.cs.ai.behavior.oomdp.planning.stochastic.montecarlo.uct.UCTStateNode.UCTStateConstructor;
...
public class UCT extends OOMDPPlanner {
	...
	public void planFromState(State initialState) {
		...
		while(!this.stopPlanning()){
			...
			if(nu - lastNumUnique > 0){
				System.out.println("" + numRollOutsFromRoot + "; unique states: " + nu  + "; tree size: " + treeSize + "; total visits: " + numVisits);
				lastNumUnique = nu;
			}
	...
	public double treeRollOut(UCTStateNode node, int depth, int childrenLeftToAdd){
		...
		if(tf.isTerminal(node.state.s)){
			...
			System.out.println(numRollOutsFromRoot + " Hit terminal at depth: " + depth);
			return 0.;
		}
		...
		if(foundGoalOnRollout && anode != this.bestReturnAction(node)){
			System.out.println("Not propagated correctly...");
		}
