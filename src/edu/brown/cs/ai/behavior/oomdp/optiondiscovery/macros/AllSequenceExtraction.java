...
public class AllSequenceExtraction {
	...
	public static MacroAction extractFullPlanMacro(State s, Policy p, TerminalFunction tf, String macroName){
		
		p.evaluateMethodsShouldDecomposeOption(false);
		EpisodeAnalysis ea = p.evaluateBehavior(s, new NullReward(), tf);
		MacroAction ma = new MacroAction(macroName, ea.actionSequence);
	...
	public static List<MacroAction> extractAllPrimtiveMacros(State s, Policy p, TerminalFunction tf, String macroBaseName){
		
		p.evaluateMethodsShouldDecomposeOption(false);
		EpisodeAnalysis ea = p.evaluateBehavior(s, new NullReward(), tf);
		
		List <List<GroundedAction>> allSubs = getAllNonMacroSubSequences(ea.actionSequence);
		List <MacroAction> macros = new ArrayList<MacroAction>(allSubs.size());
		int i = 0;
		for(List<GroundedAction> subSeq : allSubs){
			MacroAction m = new MacroAction(macroBaseName+":"+i, subSeq);
			macros.add(m);
			i++;
		}
	...
	protected static List <List<GroundedAction>> getAllNonMacroSubSequences(List <GroundedAction> actionSequence){
		
		List <List <GroundedAction>> allSubs = new ArrayList<List<GroundedAction>>();
		
		for(int i = 0; i < actionSequence.size(); i++){
			if(actionSequence.get(i).action.isPrimitive()){
				
				List <GroundedAction> subSeq = new ArrayList<GroundedAction>();
				for(int j = i; j < actionSequence.size(); j++){
					GroundedAction ga = actionSequence.get(j);
					i = j;
				...
				if(subSeq.size() > 1){
					...
					if(DPrint.mode(837694)){
						for(GroundedAction ga : subSeq){
							DPrint.c(837694, ga.toString() + "; ");
						}
						DPrint.cl(837694, "");
					}
				}
